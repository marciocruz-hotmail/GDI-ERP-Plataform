...
**Fonte:** `2026_05_20_performance-audit-erp.md` (auditoria estática, sem medições em produção)  
...
| Typeahead pedidos | `.cursor/context/2026_05_20_lookups-typeahead-ajax-pedidos.md` |
| Auditoria completa | `2026_05_20_performance-audit-erp.md` |
...
- [x] **G-PERF-20.0.2** Inventário views → flags.
  - **Script:** `python Scripts/2026_05_20_gdi_inventory_page_scripts.py`
  - **Saída:** `.cursor/context/2026_05_20_layout-scripts-inventario.json` (239 views; 136 DT; 14 S2; 2 jstree; 3 Tempus; 47 hosts `mainModal`).
- [x] **G-PERF-20.0.3** Matriz hub → modais → libs.
  - **Doc:** `.cursor/context/2026_05_20_layout-scripts-matriz-modais.md`
- [x] **G-PERF-20.0.4** Contrato `GdiPageScriptsFlags` + `Lib/GdiPageScripts.cs` (filter **não** registado; `_Layout` inalterado).
  - **Doc:** `.cursor/context/2026_05_20_layout-scripts-contrato-flags.md`
...
- [x] **G-PERF-20 Fase 4** — opt-out lotes A (7 hubs) + B (Treinamentos, Pedidos portal) + C (CreateEdit/forms sem DT/S2); `VersionERP` **2026.51.11** — doc `2026_05_20_layout-scripts-fase4-optout.md`.
  - **G-PERF-M02:** ver `2026_05_20_perf-m02-resultado.md` + script `2026_05_20_gdi_perf_m02_network_baseline.py`.

- [x] **G-PERF-20f** — lazy load `#mainModal` (`GdiLoadScriptOnce`, patch `jQuery.load`); `VersionERP` **2026.51.12** — doc `2026_05_20_layout-scripts-fase5-lazy-modal.md`.

**G-PERF-20 (layout scripts):** fases 0–4 + **20f** concluídas no código.

### G-PERF-M02 — Medição pós-layout scripts

- [x] **G-PERF-M02a** Estimativa estática (assets layout × gzip proxy) — `2026_05_20_perf-m02-resultado.json` (2026-05-20).
...
- [ ] **G-PERF-M02b** DevTools **live** homologação (Finish + transferred full navigation) — `python Scripts/2026_05_20_gdi_perf_m02_network_baseline.py --live` + smoke modais hub.
...
- [x] **PERF-004** (PERF-003) Typeahead Ajax no filtro de cliente (`IndexPedido`).
  ...
  - **Ação mínima:**
    ...
    2. Replicar padrão CreateEdit: `LookupSearchQueries.ComboPlaceholderCliente()` + `data-gdi-lookup-url` / `gdi-select2.js` apontando para action existente (`GetClientesLookup` ou equivalente documentado em `2026_05_20_lookups-typeahead-ajax-pedidos.md`).
...
- [x] **PERF-007** Inventariar e corrigir **lote 2** de `GetDados` com `allRecords.ToList()` + `Skip`.
  ...
  - **Resultado (2026-05-20):** `Lib/LibDataTableSqlPaging.cs`; lote 2 paginado no SQL/EF; inventário `Scripts/2026_05_20_gdi_inventory_datatables_memory_paging.py` + `.cursor/context/2026_05_20_perf007-lote2-inventario.md` — **8 pendentes lote 3**, **1 aceite** (`GetDadosInvoicesItensEspelhoDigital`).
...
- [x] **PERF-012** (PERF-009) Compressão e cache de estáticos no IIS.
  ...
  - **Resultado (2026-05-20):** `.cursor/context/2026_05_20_perf012-iis-static-compression-cache.md`; `Web.Release.config` (`urlCompression` + `clientCache` 7d).
...
- [x] **PERF-015** (PERF-008) Reduzir `MAX()` em `IsTableUpdate` para combos frequentes.
  ...
  - **Resultado (2026-05-20):** `DateTimeLastVerified` + TTL 5/15 min; cache hit antes de `IsTableUpdate`; `ResetTableUpdateVerification` em `InvalidateForTable`. Ver `2026_05_20_perf015-is-table-update-ttl.md`.
