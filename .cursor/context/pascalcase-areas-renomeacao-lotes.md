...
**Já concluído:** B1–B2d (2026-05-19) — views `modal*`, models `cst*` em `Areas` + `Models/CstTenant` + `Robos/SintegraWS`. **Zero** `public class cst*` no repo.
...
| `Scripts/gdi_remove_remaining_modal_icons.ps1` | 3 nomes de ficheiro |
| `Scripts/gdi_check_specific_modals.ps1` | action `modalAtualizarFaturamentoGestorFranquia` → `ModalAtualizar...` (só no comentário/array de verificação; action real no controller já é PascalCase) |
...
- [ ] `python Scripts/gdi_verify_csproj_gdi_helpers.py` (se views com helpers Gdi tocadas)
