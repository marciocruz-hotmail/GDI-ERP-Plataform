...
**Fase 16–17 — `NfeController` + e-Notas:** `Index`, `Edit`, `GetDados` / `GetDadosNfeLogs` (DataTables + `JsonDataTableException`); modais Ajax com `{ success, msg }` e **`RoboEnotasNFE`** (gerar/atualizar/cancelar/sincronizar — ver `.cursor/context/2026_05_26_nfe-enotas-arquitetura.md`). **`PortalVendedorController` removido** (2026-05-19); roles `g_PortalVendedor_*` mantidas só em troca de senha vendedor (`UsuariosController`, `TokenAcesso` V). **`AtendimentosController`** — **`JsonDataTableException`**; **`Nfe/CreateEdit`** logs com **`xhr.dt`**.
